...
#ifndef CATLASS_EPILOGUE_BLOCK_COMBINE_SCALE_HPP
#define CATLASS_EPILOGUE_BLOCK_COMBINE_SCALE_HPP
...
public:
    ...
    __aicore__ inline void operator()(
        ...
        uint32_t headSizeV,
        __gm__ SplitInfoType &splitInfo,
        ...
        for (uint32_t process = subBlockID; process < kvSplitCoreNum * 2; process += subBlockNum) {
            ...
            uint32_t headStartIndx = splitInfo[process/2].headStartIdx;
            uint32_t headEndIndx = splitInfo[process/2].headEndIdx;
            uint32_t qStartIndx = splitInfo[process/2].qStartIdx;
            uint32_t qEndIndx = splitInfo[process/2].qEndIdx;
            ...
            uint32_t addrLOffset = vectorsubBlockID == 0 ? splitInfo[process/2].lseTaskOffset : splitInfo[process/2].lseTaskOffset + sum_former;
            uint32_t addrOOffset = vectorsubBlockID == 0 ? splitInfo[process/2].oTaskOffset : splitInfo[process/2].oTaskOffset + sum_former * headSizeV;

            uint32_t prevQSeqlenSum = 0;
            if (inputLayoutTND) {
                prevQSeqlenSum = static_cast<uint32_t>(gActualQseqlen.GetValue(batchIdx));
            } else {
                prevQSeqlenSum = batchIdx * maxQSeqlen;
            }
            ...
            uint32_t splitNum = splitInfo[process/2].splitNum;
