...
// autogen/generate_kernels.py) so the FAInfer instantiations compile in
...
// dtype / paged / causal / softcap and launches the matching FAInfer<...>.
// IS_TND selects the input layout: false => BSND (mha_fwd_kvcache, mha_fwd),
// true => TND (mha_varlen_fwd). Flash-decode is NOT a template parameter: it is
// a runtime tiling flag (FAInferTilingData.flashDecodeFlag) so the AICPU
// scheduler-metadata path can decide it on device.
//
