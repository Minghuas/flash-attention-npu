...
// scheduler-metadata path where no D2H sync is allowed. An over-long window
// only collapses to "no window" earlier than the host branch would, which is
// semantically identical (a window covering the whole sequence masks nothing).
