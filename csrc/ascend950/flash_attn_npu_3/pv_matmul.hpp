...
public:
    ...
    BlockMmadTla(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart, BlockMmadPVTileHelper &pvL1TileHelper)
    {
        ...
        l1ATileK = pvL1TileHelper.pvL1TileKLeft;
        l1BTileK = pvL1TileHelper.pvL1TileKRight;
                    ...
                    uint64_t prefixSumL0AStages, uint64_t prefixSumL0BStages)
    {
        ...
        // fwd crossCoreSync from online sm to pv
        WaitCrossCoreSync<4, PIPE_MTE1>(softmaxReadyFlag);
        ...
        for (uint32_t nL0Itr = 0; nL0Itr < nL0LoopNum; nL0Itr++) {
            ...
            for (uint32_t mL0Itr = 0; mL0Itr < mL0LoopNum; mL0Itr++) {
                ...
                for (uint32_t kL0Itr = 0; kL0Itr < kL0LoopNum; kL0Itr++) {
                    ...
                    if constexpr (!ENABLE_DN_) {
                        ...
                        loadData2DParamsA.ifTranspose = 1;
                        loadData2DParamsA.mStep = RoundUp(l0TileKAct, 16) / 16;
                        loadData2DParamsA.kStep = RoundUp(l0TileMAct, 16) / 16;
                        loadData2DParamsA.srcStride = RoundUp(l0TileKAct, 16) / 16;
                        loadData2DParamsA.dstStride = RoundUp(l0TileMAct, 16) / 16;
                        AscendC::LoadData(l0ATensor[l0ABufId], l1ATensorTla.data(), loadData2DParamsA);
                    }
                    AscendC::SetFlag<AscendC::HardEvent::MTE1_M>(l0AEventId);
                    // reverse crossCoreSync for P
                    if ((mL0Itr == mL0LoopNum - 1) && (nL0Itr == nL0LoopNum - 1) && (kL0Itr == kL0LoopNum - 1)) {
                        SetCrossCoreSync<4, PIPE_MTE1>(softmaxReadyFlag);
                    }
            ...
            if (nL0Itr == 0) {
                ...
                WaitCrossCoreSync<4, PIPE_FIX>(pvReadyFlag);
            }
