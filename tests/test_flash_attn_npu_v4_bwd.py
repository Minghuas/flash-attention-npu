...
"""
FlashAttention v4 反向 pytest（from_fwd_v4：FAI fwd + FAG_v4 bwd）。

- 正向：flash_attn_varlen_func 得到 out / softmax_lse，不与标杆比较
...
用法:
  pytest tests/test_flash_attn_npu_v4_bwd.py -v
  pytest tests/test_flash_attn_npu_v4_bwd.py -k swa -v
"""
...
if "Ascend950" in (torch_npu.npu.get_device_name() if torch_npu.npu.device_count() > 0 else ""):
    pytest.skip("flash_attn_varlen_func not on Ascend950", allow_module_level=True)
...
test_cases_varlen = [
    (torch.bfloat16, 2, 4, 2, 128, 128, 64, True, 0.0),
...
test_cases_varlen_swa = [
    (torch.bfloat16, 3, 4, 4, 1024, 1024, 128, True, 512, 0, 0.0),
