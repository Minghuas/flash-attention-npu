...
test_cases_bsnd_swa = [
    ...
    # ---- ALiBi ----
    (torch.bfloat16, 1, 1, 1, 1024, 1024, 128, False, 0, 256, 0.0, True),
...
test_cases_varlen = [
    ...
    (torch.bfloat16, 1, 1, 1, 512, 1024, 128, True, 0.0, True),
    (torch.float16, 1, 5, 1, 512, 512, 128, True, 0.0, True),
    (torch.float16, 1, 5, 1, 777, 888, 192, False, 0.0, True),
...
def test_fa_varlen_bwd(
    ...
    # varlen alibi: one slope per head; batch_size=1 under the hood
    alibi_slopes = make_alibi_slopes(1, num_heads).npu() if use_alibi else None
