...
| ⑥ | Sk=80 罕见竞争残余 | 1/5 轮 69 错 max 0.44（b1 s0 h6） | ✅ 已修（结构性）：v3 换通用 Pingpong 引擎（fork splitb_bm_pingpong.hpp，A/B 面 L1 双缓冲+槽位轮转）后 **B4/B16 Sq32 Sk96 ×8 全 PASS 逐迭代恒定**（#46.6，2026-08-30）。t19 曾实证 S@100 撕裂比值 (h+2)/(h+1)=Q(h+1)·K(h)（#45.3）；EV5+PipeBarrier 守卫无效（#44.53j 证伪，疑 LoadData 不在 MTE1 域）。pytest 残 1/2097152 单点（小值 atol 边缘，既有环境性）。 |
