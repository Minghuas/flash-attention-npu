...
def run_one(batch, seqlen, nheads, headdim, dtype, causal, softcap,
    ...
    k_fa = torch.randn(batch, seqlen, nheads, headdim, dtype=dtype, device=device).contiguous()
    v_fa = torch.randn(batch, seqlen, nheads, headdim, dtype=dtype, device=device).contiguous()
...
def main():
    ...
    ap.add_argument("--batch", type=int, nargs="+", default=[1, 4, 16, 64, 256, 320],
    ...
    # ap.add_argument("--seqlen", type=int, nargs="+", default=[32, 64, 128, 256, 512, 2048], help="seqlen 列表（默认含小 seqlen）")
    ...
    ap.add_argument("--repeat", type=int, default=100,
                    help="单点重复次数取平均值（2026-08-14 用户要求 ≥100；此前数据为 repeat=10/20）")
    ...
    for b in args.batch:
        for s in args.seqlen:
            ...
            rows.append(row)
            if not first_err_printed and row.get("fa_v2_error"):
                print(f"# [fa_v2 首个错误] b={b} s={s}: {row['fa_v2_error']}")
                first_err_printed = True
            if not first_err_printed and row.get("baseline_error"):
                print(f"# [baseline 首个错误] b={b} s={s}: {row['baseline_error']}")
                first_err_printed = True
