...
def make_alibi_slopes(batch_size, num_heads):
    ...
    # _h = torch.tensor([1*(h + 1) for h in range(num_heads)], dtype=torch.float32)
    # _h = torch.tensor([100*(h + 1) for h in range(num_heads)], dtype=torch.float32)
    # _h = torch.tensor([0.00001*(h + 1) for h in range(num_heads)], dtype=torch.float32)
...
# ---------- INDEPENDENT simple reference (full fp32, no chunking, plain softmax) ----------
# Deliberately different implementation from ref_flash_attention (which chunks KV by 512
# and uses online-softmax). If simple_ref == ref_flash_attention, the chunked ref is correct.
def simple_ref_attention(q, k, v, scale, alibi_slopes=None, is_causal=False):
    """
    q:[Sq,H,D], k:[Sk,KVH,D], v:[Sk,KVH,D] (per-batch, cpu).
    Returns out:[Sq,H,D], lse:[H,Sq] — same shapes as ref_flash_attention.
    """
    Sq, H, D = q.shape
    Sk, KVH, _ = k.shape
    group = H // KVH
    kv_idx = [h // group for h in range(H)]
    qf = q.float()                                  # [Sq,H,D]
    kf = k[:, kv_idx, :].float()                    # [Sk,H,D]  (GQA broadcast)
    vf = v[:, kv_idx, :].float()                    # [Sk,H,D]
    # score[h,q,k] = sum_d qf[q,h,d]*kf[k,h,d]
    score = torch.einsum('qhd,khd->hqk', qf, kf) * scale   # [H,Sq,Sk]
    if alibi_slopes is not None:
        slopes = alibi_slopes.float().reshape(H, 1, 1)
        i_abs = (Sk - Sq) + torch.arange(Sq, dtype=torch.float32)   # right-aligned query
        j_abs = torch.arange(Sk, dtype=torch.float32)
        dist = (i_abs.reshape(1, -1, 1) - j_abs.reshape(1, 1, -1)).abs()  # [1,Sq,Sk]
        score = score + (-slopes) * dist
    if is_causal:
        cm = torch.triu(torch.ones(Sq, Sk), diagonal=Sk - Sq + 1).bool()
        score = score.masked_fill(cm.unsqueeze(0), float('-inf'))
    # full softmax (fp32)
    s_max = score.max(dim=-1, keepdim=True)[0]      # [H,Sq,1]
    P = torch.exp(score - s_max)                    # [H,Sq,Sk]
    rowsum = P.sum(dim=-1, keepdim=True)            # [H,Sq,1]
    lse = (torch.log(rowsum) + s_max).squeeze(-1)   # [H,Sq]
    P = P / rowsum
    out = torch.einsum('hqk,khd->hqd', P, vf)       # [H,Sq,D]
    return out.permute(1, 0, 2), lse                 # [Sq,H,D], [H,Sq]
...
def main(args):
    ...
    kernel_slopes = slopes if use_alibi else None

    print(f"==> Q=K=0 V=[0..Sk-1], batch={B} H={H} kvH={KVH} Sq={Sq} Sk={Sk} D={D} causal={args.causal} alibi={use_alibi}")
    if use_alibi:
    ...
    out, lse, _ = flash_attn_func(q, k, v, dropout_p=0.0, softmax_scale=scale,
                                  ...
                                  softcap=0.0, alibi_slopes=kernel_slopes, return_attn_probs=True)
    ...
    for b in range(B):
        ...
        ref_o, ref_l = ref_flash_attention(
            ...
            softcap=0.0, alibi_slopes=(s_cpu if use_alibi else None), is_causal=args.causal)
    ...
    # ----- INDEPENDENT simple reference (proof that ref_flash_attention is correct) -----
    simple_out_list = []
    simple_lse_list = []
    for b in range(B):
        so, sl = simple_ref_attention(
            q[b].cpu(), k[b].cpu(), v[b].cpu(), scale,
            alibi_slopes=(slopes_cpu[b] if use_alibi else None), is_causal=args.causal)
        simple_out_list.append(so)
        simple_lse_list.append(sl)
    simple_out = torch.stack(simple_out_list, dim=0)   # [B,Sq,H,D]
    simple_lse = torch.stack(simple_lse_list, dim=0)   # [B,H,Sq]

    ref_vs_simple_out = (ref_out.float() - simple_out.float()).abs().max().item()
    ref_vs_simple_lse = (ref_lse.float() - simple_lse.float()).abs().max().item()
    print(f"    [SANITY] ref_flash_attention vs simple_ref: out_maxdiff={ref_vs_simple_out:.6e}  lse_maxdiff={ref_vs_simple_lse:.6e}")
    if ref_vs_simple_out < 1e-2 and ref_vs_simple_lse < 1e-2:
        print(f"    [SANITY] ✅ TWO independent refs AGREE → ref is correct. Any kernel mismatch is a KERNEL bug.")
    else:
        print(f"    [SANITY] ⚠ refs DISAGREE → ref itself may be wrong!")
    print()
...
if __name__ == "__main__":
    ...
    p.add_argument("--no-alibi", action="store_true", help="disable ALiBi (isolation test)")
    args = p.parse_args()
    main(args)
