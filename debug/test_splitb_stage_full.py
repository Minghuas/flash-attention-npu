...
kernel 侧（FLASH_ATTN_SPLITB_DUMP=1，自动单核）每个阶段整区一次 DumpTensor：
...
def main():
    ...
    print("[RUN] 开始跑 kernel：请用 `python debug/test_splitb_stage_full.py --batch %d --iters %d%s "
          "2>&1 | tee debug/log/t4x.log` 收集日志，随后 --log 比对"
          % (B, args.iters, " --softmax-only" if args.softmax_only else ""), flush=True)
    run_kernel(args.iters, args.softmax_only)
