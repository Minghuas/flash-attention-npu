...
if __name__ == "__main__":
    ...
    print(f"ref O(b1,0:4,h0,:4) = {o_ref[1, :4, 0, :4].tolist()}")  # 参考值预览
    # 坏区中间量参考（b2 h0 s48-63 = t26/t27 实证坏行区，与 dump 探针 desc=111/212/223/224/321/412 对齐）
    print(f"ref S_raw(b1,h0,s48) = {scores_raw[1, 0, 48, :8].tolist()}")
    print(f"ref P_unorm(b1,h0,s63,:8) = {p_unorm[1, 63, 0, :8].tolist()}")
    print(f"ref max(b1,h0,s60:64) = {maxv[1, 60:64, 0].tolist()}")
    print(f"ref sum(b1,h0,s60:64) = {sumv[1, 60:64, 0].tolist()}")
    print(f"ref OTmp(b1,h0,s63,:4) = {otmp[1, 63, 0, :4].tolist()}")
    print(f"ref O(b1,h0,s63,:4) = {o_ref[1, 63, 0, :4].tolist()}")
    npass = 0
    for itr in range(20):      # 1MB 预算 = 单次 kernel 调用，迭代次数无关（聚焦版每轮 ≤64KB）
